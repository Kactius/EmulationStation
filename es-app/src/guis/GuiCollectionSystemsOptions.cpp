...
GuiCollectionSystemsOptions::GuiCollectionSystemsOptions(Window* window) : GuiComponent(window), mMenu(window, "CONFIGURAR COLECCIÓNES")
{
	initializeMenu();
}
...
void GuiCollectionSystemsOptions::initializeMenu()
{
	...
	{
		addEntry("CREAR NUEVA COLECCIÓN A PARTIR DEL TEMA", 0x777777FF, true,
		[this, unusedFolders] {
			auto s = new GuiSettings(mWindow, "SELECCIONE LA CARPETA DEL TEMA");
			std::shared_ptr< OptionListComponent<std::string> > folderThemes = std::make_shared< OptionListComponent<std::string> >(mWindow, "SELECCIONE LA CARPETA DEL TEMA", true);
	...
	row.makeAcceptInputHandler([this, createCustomCollection] {
		mWindow->pushGui(new GuiTextEditPopup(mWindow, "Nombre de la nueva colección", "", createCustomCollection, false));
	});
	...
	mMenu.addWithLabel("AGRUPAR LAS COLECCIONES PERSONALIZADAS SIN TEMA", bundleCustomCollections);
	...
	mMenu.addWithLabel("ORDENAR LAS COLECCIONES PERSONALIZADAS Y SISTEMAS", sortAllSystemsSwitch);
	...
	mMenu.addWithLabel("MOSTRAR EL NOMBRE DEL SISTEMA EN LAS COLECCIONES", toggleSystemNameInCollections);

	if(CollectionSystemManager::get()->isEditing())
	{
		row.elements.clear();
		row.addElement(std::make_shared<TextComponent>(mWindow, "FINALIZAR LA EDICIÓN DE LA COLECCIÓN '" + Utils::String::toUpper(CollectionSystemManager::get()->getEditingCollection()) + "' COLLECTION", Font::get(FONT_SIZE_MEDIUM), 0x777777FF), true);
		row.makeAcceptInputHandler(std::bind(&GuiCollectionSystemsOptions::exitEditMode, this));
		mMenu.addRow(row);
	}

	mMenu.addButton("VOLVER", "volver", std::bind(&GuiCollectionSystemsOptions::applySettings, this));
...
void GuiCollectionSystemsOptions::addSystemsToMenu()
{
	...
	autoOptionList = std::make_shared< OptionListComponent<std::string> >(mWindow, "SELECCIONAR COLECCIONES", true);
	...
	customOptionList = std::make_shared< OptionListComponent<std::string> >(mWindow, "SELECCIONAR COLECCIONES", true);
	...
	mMenu.addWithLabel("COLECCIONES PERSONALIZADAS DE JUEGOS", customOptionList);
}
...
std::vector<HelpPrompt> GuiCollectionSystemsOptions::getHelpPrompts()
{
	...
	prompts.push_back(HelpPrompt("b", "volver"));
	return prompts;
}
