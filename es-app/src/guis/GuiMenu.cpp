...
GuiMenu::GuiMenu(Window* window) : GuiComponent(window), mMenu(window, "MENÚ PRINCIPAL"), mVersion(window)
{
	...
	if (isFullUI)
		addEntry("AJUSTES DE COLECCIÓNES", 0x777777FF, true, [this] { openCollectionSystemSettings(); });

	if (isFullUI)
		addEntry("OTRAS CONFIGURACIONES", 0x777777FF, true, [this] { openOtherSettings(); });

	if (isFullUI)
		addEntry("CONFIGURAR CONTROLES (MAPEADO)", 0x777777FF, true, [this] { openConfigInput(); });
...
void GuiMenu::openScraperSettings()
{
	...
	s->addWithLabel("SCRAPEAR CLASIFICACIONES", scrape_ratings);
...
void GuiMenu::openSoundSettings()
{
	...
	if (UIModeController::getInstance()->isUIModeFull())
	{
		...
		s->addWithLabel("ACTIVAR AUDIO DE LOS VÍDEOS", video_audio);
		s->addSaveFunc([video_audio] { Settings::getInstance()->setBool("VideoAudio", video_audio->getState()); });

#ifdef _RPI_
		// OMX player Audio Device
		auto omx_audio_dev = std::make_shared< OptionListComponent<std::string> >(mWindow, "REPRODUCTOR OMX DE AUDIO", false);
		...
		s->addWithLabel("REPRODUCTOR OMX DE AUDIO", omx_audio_dev);
...
void GuiMenu::openUISettings()
{
	...
	s->addSaveFunc([ UImodeSelection, window]
	{
		...
		{
			std::string msg = "Está cambiando la interfaz del usuario a un modo restringido:\n" + selectedMode + "\n";
			msg += "Esto ocultará la mayoría de las opciones del menú para prevenir cambios en el sistema.\n";
			msg += "Para desbloquear y volver a la interfaz del usuario completa, introduzca este código: \n";
	...
	screensaver_row.addElement(std::make_shared<TextComponent>(mWindow, "CONFIGURACIÓN SALVAPANTALLAS", Font::get(FONT_SIZE_MEDIUM), 0x777777FF), true);
	...
	s->addWithLabel("TRANSICIÓN CARRUSEL", move_carousel);
	...
	if(!themeSets.empty())
	{
		...
		auto theme_set = std::make_shared< OptionListComponent<std::string> >(mWindow, "ELEGIR TEMA", false);
		for(auto it = themeSets.cbegin(); it != themeSets.cend(); it++)
			theme_set->add(it->first, it->first, it == selectedSet);
		s->addWithLabel("ELEGIR TEMA", theme_set);
	...
	// GameList view style
	auto gamelist_style = std::make_shared< OptionListComponent<std::string> >(mWindow, "VISTA DEL LISTADO DE JUEGOS", false);
	...
	s->addWithLabel("VISTA DEL LISTADO DE JUEGOS", gamelist_style);
...
{
	auto s = new GuiSettings(mWindow, "OTRAS CONFIGURACIONES");

	// maximum vram
	auto max_vram = std::make_shared<SliderComponent>(mWindow, 0.f, 1000.f, 10.f, "Mb");
	max_vram->setValue((float)(Settings::getInstance()->getInt("MaxVRAM")));
	s->addWithLabel("VRAM LIMITE", max_vram);
	...
	// gamelists
	auto gamelistsSaveMode = std::make_shared< OptionListComponent<std::string> >(mWindow, "GUARDAR METADATOS", false);
	...
	s->addWithLabel("GUARDAR METADATOS", gamelistsSaveMode);
	...
	s->addWithLabel("UNICAMENTE ANALIZAR LISTA DE JUEGOS", parse_gamelists);
	s->addSaveFunc([parse_gamelists] { Settings::getInstance()->setBool("ParseGamelistOnly", parse_gamelists->getState()); });

	auto local_art = std::make_shared<SwitchComponent>(mWindow);
	local_art->setState(Settings::getInstance()->getBool("LocalArt"));
	s->addWithLabel("BÚSQUEDA DE ARTE LOCAL", local_art);
	...
	s->addWithLabel("MOSTRAR FRAMERATE", framerate);
...
{
	Window* window = mWindow;
	window->pushGui(new GuiMsgBox(window, "¿ESTÁ SEGURO DE QUERER CONFIGURAR LOS CONTROLES (MAPEAR)?", "SÍ",
		[window] {
...
void GuiMenu::openQuitMenu()
{
	...
	if (UIModeController::getInstance()->isUIModeFull())
	{
		row.makeAcceptInputHandler([window] {
			window->pushGui(new GuiMsgBox(window, "¿REALMENTE REINICIAR?", "SÍ",
				[] {
		...
		if(Settings::getInstance()->getBool("ShowExit"))
		{
			row.elements.clear();
			row.makeAcceptInputHandler([window] {
				window->pushGui(new GuiMsgBox(window, "¿REALMENTE SALIR?", "SÍ",
					[] {
					Scripting::fireEvent("quit");
					quitES();
				}, "NO", nullptr));
	...
	row.elements.clear();
	row.makeAcceptInputHandler([window] {
		window->pushGui(new GuiMsgBox(window, "¿REALMENTE APAGAR EL SISTEMA?", "SÍ",
			[] {
...
void GuiMenu::openScreensaverOptions() {
	mWindow->pushGui(new GuiGeneralScreensaverOptions(mWindow, "AJUSTES DEL SALVAPANTALLAS"));
}
...
std::vector<HelpPrompt> GuiMenu::getHelpPrompts()
{
	...
	prompts.push_back(HelpPrompt("up/down", "elegir"));
